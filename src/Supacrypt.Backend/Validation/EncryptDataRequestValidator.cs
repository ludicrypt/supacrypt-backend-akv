...
public class EncryptDataRequestValidator : AbstractValidator<EncryptDataRequest>
{
    public EncryptDataRequestValidator()
    {
        RuleFor(x => x.Version)
        ...
        RuleFor(x => x.Plaintext)
            .ValidDataSize(maxSizeBytes: 1024 * 1024);

        RuleFor(x => x.Parameters)
            .NotNull().WithMessage("Encryption parameters are required");
        ...
        When(x => x.Parameters?.RsaParams != null, () =>
        {
            RuleFor(x => x.Parameters.RsaParams.PaddingScheme)
                ...
                .WithMessage("RSA padding scheme must be PKCS#1 or OAEP for encryption");
