...
public class VerifySignatureRequestValidator : AbstractValidator<VerifySignatureRequest>
{
    public VerifySignatureRequestValidator()
    {
        ...
        RuleFor(x => x.Signature)
            .NotNull().WithMessage("Signature is required")
            .Must(sig => sig.Length > 0).WithMessage("Signature cannot be empty")
            .Must(sig => sig.Length <= 1024).WithMessage("Signature size cannot exceed 1024 bytes");

        RuleFor(x => x.Parameters)
            .NotNull().WithMessage("Verification parameters are required");
        ...
        When(x => x.Parameters?.RsaParams != null, () =>
        {
            RuleFor(x => x.Parameters.RsaParams.PaddingScheme)
                ...
                .WithMessage("RSA padding scheme must be PKCS#1 or PSS for verification");
